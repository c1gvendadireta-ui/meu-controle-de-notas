    ...
    with tab1:
        ...
        if st.button("Enviar Nota"):
            if foto and id_despesa:
                try:
                    # Forçamos o valor a manter 2 casas decimais como string
                    valor_formatado = f"{valor:.2f}"
                    nome_personalizado = f"{id_despesa}_{data}_{valor_formatado}"
                    ...
                    sheet_notas.append_row([id_despesa, str(data), valor_formatado, estabelecimento, url_foto, st.session_state.logged_in_user, "Ativo"])
